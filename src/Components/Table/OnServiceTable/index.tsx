...
interface TablesProps {
  ...
  data: ITableData[];
  title: string;
  onWorker: (user: ITableData) => void;
  loading: boolean;
  disabled?: boolean;
}

export const OnServiceTable: React.FC<TablesProps> = ({
  columnTitles,
  data,
  onWorker,
  loading,
              ...
              {data && data?.length === 0 ? (
                ...
                data?.map((row) => (
                  <StyledTableRow key={row.id}>
                    ...
                    <StyledTableCell>{Capitalize(row.surName)}</StyledTableCell>

                    <StyledTableCell>
                      <BtnMini
                        disabled={disabled}
                        onClick={() => onWorker(row)}
                        type="onWorker"
                      />
                    </StyledTableCell>
                    <StyledTableCell>
                      <BtnMini type="offWorker" disabled={true} />
                    </StyledTableCell>
                  </StyledTableRow>
                ))
              )}
